...
export default function LoginScreen({ handleLogin, setScreen, language }) {
  ...
  const handleGoogleCallback = async (tokenResponse) => {
    ...
    try {
      ...
      showAlert(L('Error', 'Lỗi'), L('Cannot connect. Please try again.', 'Không thể kết nối. Vui lòng thử lại.'));
    } finally {
      setSocialLoading('');
    }
  };
  ...
  const doLogin = async () => {
    ...
    try {
      ...
      showAlert(L('Error', 'Lỗi'), L('Server connection error', 'Lỗi kết nối máy chủ'));
    } finally {
      setLoading(false);
    }
  };
